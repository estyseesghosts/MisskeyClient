...
class MisskeyAuth(private val api: MisskeyApi) : AuthGateway {
    ...
    override suspend fun complete(pending: PendingLogin): LoginSession {
        ...
        val result = JSONObject(api.post(pending.origin, "miauth/${pending.id}/check"))
        ...
        return LoginSession(pending.origin, token, user)
    }
}
...
object AuthCallback {
    fun matches(value: String, pending: PendingLogin, now: Long): Boolean = runCatching {
        val uri = URI(value)
        val sessions = uri.rawQuery.orEmpty().split('&').filter { it.startsWith("session=") }
        uri.scheme == "palustris" && uri.host == "auth" && uri.path == "/misskey" &&
            uri.userInfo == null && uri.port == -1 && uri.fragment == null && pending.isFresh(now) &&
            sessions.size == 1 && sessions.single().removePrefix("session=") == pending.id
    }.getOrDefault(false)
}
