...
| --- | --- | --- | --- | --- |
| C-01 | Step 2 | Make connected identity coherent. Resolve one accepted context. Remove the `sourceFactory.create` fallback. | Every source-backed feature receives values from one accepted connected lifetime. | implemented, test verified in the working tree. No commit yet. |
...
| Slice | Report step | Scope | Exit | Status |
| --- | --- | --- | --- | --- |
| C-02 | Step 3 | Repair ViewModel lifetime and route re-entry. Add an explicit connected-entry store and lifecycle owner. | Retired owners cannot publish. Re-entered features never receive stopped owners. | pending |
...
**C-02 — Repair ViewModel lifetime and route re-entry.**

Not started. Work from `progressreport.md` section 3 step 3. Commit C-01 first.

C-01 files have no commit. Do not start C-02 while C-01 is uncommitted unless the user
authorizes it.

## Files Involved For C-02
...
- `app/src/main/java/me/foxtails/palustris/ui/SavedCollectionsHost.kt`
- `app/src/main/java/me/foxtails/palustris/ui/profile/ProfileHost.kt`
- `app/src/main/java/me/foxtails/palustris/ui/thread/ThreadHost.kt`
- `app/src/main/java/me/foxtails/palustris/ui/directmessages/DirectMessagesHost.kt`
- `app/src/main/java/me/foxtails/palustris/ui/notifications/NotificationsHost.kt`
- `app/src/main/java/me/foxtails/palustris/ui/notifications/NotificationSettingsHost.kt`
- `app/src/main/java/me/foxtails/palustris/ui/emoji/EmojiHost.kt`
- `app/src/main/java/me/foxtails/palustris/ui/settings/SettingsOverlayHost.kt`

Proposed: a connected-entry lifetime owner and host lifecycle tests.
...
Test these cases for C-01:

- Account A to account B. Covered by `ConnectedSessionContextTest.switchingAccountReplacesTheWholeContext`.
- Same-account session replacement. Covered by `sameAccountReplacementBumpsRevisionAndRetiresTheOldRegistration`.
- Registered source temporarily unavailable. Covered by `aRetiredRegistrationCannotResolveToTheAcceptedSource`.
- Delayed registration from a retired session. Not applicable. `register` is synchronous.
  The retired token is unresolvable, per the test above.
- Recomposition without new source construction. Covered by
  `profileUpdateKeepsTheAcceptedSourceRevisionAndGeneration`.
- Profile updates during account replacement. Covered by the same test.
- Post preferences from the previous account during a transition. Covered by the
  account and generation assertions in `switchingAccountReplacesTheWholeContext`.
...
`b629a2c` "Close cancellation and shell continuation record".

This commit is the source baseline for this task. C-01 is implemented and test verified in the
working tree. C-01 has no commit yet.
